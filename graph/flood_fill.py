...
directly adjacent (pixels that share a side with the original pixel, either horizontally or vertically) 
    ...
    def floodFill(self, image: list[list[int]], sr: int, sc: int, color: int) -> list[list[int]]:
        ...
        while queue:
            ...
            for row_offset, col_offset in offsets:
                row = node_row + row_offset
                col = node_col + col_offset
                if not (0 <= row < m and 0 <= col < n):
                    continue

                if not visited[row][col] and image[row][col] == initial_color:
                    image[row][col] = color
